...
export default function Header() {
  ...
  return <>
    <div className=" top-0 right-0 z-40 flex items-center justify-between  w-full h-16 px-4 bg-[#15191c]  sm:ml-64 sm:w-[calc(100%-16rem)]">
      <div className="flex items-center">
        <button onClick={toggleSidebar} className="p-2 mr-2 text-gray-400 rounded-lg sm:hidden hover:bg-gray-700">
          <Menu className="w-6 h-6" />
        </button>
        <div
      className="flex items-center px-4 py-2 bg-[#1B2131] rounded-lg w-60"
      onClick={handleClick}
    >
      <svg
      ...
      {isEditing ? (
        ...
        <span className="text-sm font-medium text-gray-200 cursor-text">
          AI Assistant
        </span>
      )}
    </div>
