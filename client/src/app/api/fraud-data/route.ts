...
export async function GET(request: Request) {
  try {
    ...
    const fraudData = await db.collection(collectionName).findOne({});

    if (!fraudData) {
      return NextResponse.json(
        { message: "No fraud data found" },
        { status: 404 }
      );
    }
    ...
    const serializableData = {
      ...fraudData,
      _id: fraudData._id.toString(),
    };

    return NextResponse.json(serializableData);
  } catch (error) {
    console.error("API Error fetching fraud data:", error);
