...
import { Send, Bot, User, ArrowDown, X, Trash, Search, Paperclip, Image, FileText, File, HelpCircle, ChevronDown, Menu } from "lucide-react";
import { useChat, Message, FileAttachment, MessageAction } from "@/context/ChatContext";
...
export const HelpModal = ({ onClose }: { onClose: () => void }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/25 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
            <div className="bg-white dark:bg-[#1d2328] rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col animate-fadeIn">
                <div className="flex justify-between items-center p-4 border-b dark:border-gray-800">
                    <div className="flex items-center">
                        <HelpCircle className="w-6 h-6 text-[#4B65AB] dark:text-[#AEC3FF] mr-2" />
                        <h2 className="text-xl font-medium text-[#4B65AB] dark:text-[#AEC3FF]">Chat Help</h2>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-4 overflow-y-auto flex-1 dark:custom-scrollbar">
                <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800/50">
                        ...
                        <ul className="list-disc pl-5 space-y-2 text-sm text-gray-700 dark:text-gray-300">
                            ...
                            <li>Type <span className="font-mono bg-gray-100 dark:bg-gray-700 px-1 rounded">/help</span> anytime to see this help screen</li>
                        </ul>
                    </div>
                    
                    <p className="text-[#15191c] dark:text-white mb-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {helpKeywords.map((item, index) => (
                            <div 
                                key={index} 
                                className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-gray-50 dark:bg-gray-800/50 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors"
                                onClick={() => {
                            ...
                            >
                                <div className="font-medium text-[#4B65AB] dark:text-[#AEC3FF] mb-1">
                                    {item.keyword}
                                </div>
                ...
                <div className="p-4 border-t dark:border-gray-800 flex justify-end">
                    <button
                        onClick={onClose}
                        className="px-4 py-2 bg-[#4B65AB] text-white rounded-md hover:bg-[#3A5095] transition-colors"
                    >
                        Close
                    </button>
                </div>
        ...
        return (
            <div className="flex flex-wrap gap-2 mt-2">
                {attachments.map(attachment => (
                    <div 
                        key={attachment.id}
                        className="bg-gray-100 dark:bg-gray-700 rounded-md overflow-hidden border border-gray-200 dark:border-gray-600"
                    >
    ...
    return (
        <div className="fixed inset-0 bg-black/25 backdrop-blur-sm z-50 flex items-center justify-center p-4 shadow-lg">
            ...
            >
                ...
                <div className="flex justify-between items-center p-4 border-b dark:border-gray-800">
                    <div className="flex items-center">
                        ...
                        <div>
                            ...
                            <p className="hidden md:block text-sm text-gray-500 dark:text-gray-400">AI assistant providing insights and analytics to help you</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className={`p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 ${
                                searchVisible 
                                    ? "bg-gray-100 dark:bg-gray-700 text-[#4B65AB] dark:text-[#AEC3FF]" 
                ...
                {/* Search Bar */}
                {searchVisible && (
                    <div className="px-4 py-2 border-b dark:border-gray-800 bg-gray-50 dark:bg-[#1a1f24] flex items-center gap-2">
                        <div className="relative flex-1">
                            <input
                                ...
                                autoFocus
                                className="w-full py-1.5 px-3 pr-16 bg-white dark:bg-[#2a3441] border border-gray-300 dark:border-gray-700 rounded-md text-[#15191c] dark:text-white placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#4B65AB] dark:focus:ring-[#AEC3FF]"
                            />
                            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                                ...
                                <button 
                                    ...
                                    className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md disabled:opacity-50 disabled:cursor-not-allowed"
                ...
                >
                    {messages.length === 0 ? (
                        ...
                        messages.map((message) => {
                            const isSearchResult = searchVisible && 
                                searchResults.length > 0 && 
                                    ...
                                    >
                                        ...
                                        <div
                                            className={`rounded-lg px-4 py-3 ${
                                                isSearchResult 
                                                    ? "ring-2 ring-[#4B65AB] dark:ring-[#AEC3FF]" 
                                                    : ""
                                            } ${message.sender === "user"
                                                ...
                                                <div className="flex flex-wrap gap-2 mt-2">
                                                    {message.actions.map(action => (
                                                        <button
                                                            ...
                                                            className="text-xs px-3 py-1.5 rounded-md bg-[#EBF1FF] dark:bg-[#2D3748] text-[#4B65AB] dark:text-[#AEC3FF] hover:bg-[#DAE3F9] dark:hover:bg-[#3A4A63] transition-colors font-medium"
                                                        >
                                                            {action.label}
                                                        </button>
                                                    ))}
                ...
                {draftAttachments.length > 0 && (
                    <div className="px-4 py-2 border-t dark:border-gray-800 bg-white dark:bg-[#1d2328] flex gap-2 overflow-x-auto">
                        {draftAttachments.map(attachment => (
                            <div 
                                key={attachment.id}
                                className="relative flex-shrink-0 bg-gray-100 dark:bg-gray-700 rounded-md overflow-hidden border border-gray-200 dark:border-gray-600 group"
                            ...
                            >
                                ...
                                <div className="p-1">
                                    <div className="text-xs truncate font-medium max-w-[90px]">{attachment.name}</div>
                                    <div className="text-xs text-gray-500 dark:text-gray-400">
                                        {formatFileSize(attachment.size)}
                                    </div>
                                </div>
                ...
                <div className="p-4 border-t dark:border-gray-800 bg-white dark:bg-[#1d2328]">
                    <div className="flex items-center gap-2 bg-[#F0F2F5] dark:bg-[#2a3441] rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                        ...
                        <div className="relative">
                            ...
                            {showOptionsMenu && (
                                <div className="absolute bottom-full left-0 mb-2 bg-white dark:bg-[#2a3441] rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-2 w-56 z-10">
                                    <button
                                        onClick={() => {
                                            fileInputRef.current?.click();
                                            setShowOptionsMenu(false);
                                    ...
                                    >
                                        ...
                                        <span className="text-sm">Chat help</span>
                                    </button>
                                </div>
                            )}
                        </div>
                        ...
                        <button
                            ...
                            className={`flex-shrink-0 rounded-full p-2 ${(input.trim() || draftAttachments.length > 0) && !isLoading
