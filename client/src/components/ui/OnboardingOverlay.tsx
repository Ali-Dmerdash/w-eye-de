...
export default function OnboardingOverlay() {
  ...
  // If user is loaded and filesUploaded is explicitly false, do not show onboarding
  if (isLoaded && filesUploaded === false) {
    return null;
  }

  const {
    isOnboardingActive: onboardingFromContext,
    currentStep,
    steps,
    nextStep,
    prevStep,
  ...
  const [localOnboardingActive, setLocalOnboardingActive] = useState(
    onboardingFromContext
  );
  useEffect(() => {
    setLocalOnboardingActive(onboardingFromContext);
  }, [onboardingFromContext]);

  const handleClose = () => {
    setLocalOnboardingActive(false); // Hides the overlay immediately
    skipOnboarding(); // Updates context + localStorage
  };
  ...
  useEffect(() => {
    if (!localOnboardingActive || !steps[currentStep]) return;
  ...
  // Keyboard navigation
  useEffect(() => {
    if (!localOnboardingActive) return;
  ...
  }, [localOnboardingActive, currentStep, nextStep, prevStep, skipOnboarding]);

  // Wrap prevStep and nextStep in stable callbacks to avoid stale closures
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 bg-gradient-to-br from-black/60 via-purple-900/10 to-black/60 backdrop-blur-[2px] z-[9998] transition-all duration-300 ease-out pointer-events-auto ${
          localOnboardingActive ? "opacity-100" : "opacity-0"
        }`}
        style={{
          transform: localOnboardingActive
            ? "translate3d(0, 0, 0)"
            : "translate3d(0, 0, 0)",
        }}
      />
      ...
      {targetElement && targetRect && tooltipPosition.position !== "center" && (
        <div
          ...
          style={{
            ...
            width: Math.min(
              targetRect.width + 20,
              viewport.width - Math.max(0, targetRect.left - 10)
            ),
            height: Math.min(
              targetRect.height + 20,
              viewport.height - Math.max(0, targetRect.top - 10)
            ),
            boxShadow: `
      ...
      {/* Card container for overlay content */}
      <div
        className="fixed inset-0 z-[10000] flex items-center justify-center pointer-events-none"
        style={{ pointerEvents: "none" }}
      >
        <div
          className="relative bg-white dark:bg-gray-900 rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-700 max-w-lg w-full mx-4 p-0 pointer-events-auto flex flex-col"
          style={{
            minWidth: 320,
            maxWidth: 420,
        ...
        >
          {/* Tooltip and navigation buttons inside the card */}
          {/* Fixed Previous Button - Bottom Left */}
          {currentStep > 0 && (
            <button
              onClick={handlePrevStep}
              className={`absolute left-4 bottom-4 z-[10002] w-14 h-14 flex items-center justify-center bg-white/95 dark:bg-gray-800/95 border-2 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 shadow-2xl backdrop-blur-sm transition-all duration-200 ease-out hover:scale-110 hover:border-purple-400 hover:shadow-purple-500/25 rounded-full cursor-pointer group ${
                localOnboardingActive
          ...
          <button
            ...
            className={`absolute right-4 bottom-4 z-[10002] w-14 h-14 flex items-center justify-center bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white shadow-2xl transition-all duration-200 ease-out hover:scale-110 hover:shadow-purple-500/50 rounded-full cursor-pointer group ${
              localOnboardingActive
                ? "opacity-100 translate-y-0"
                : "opacity-0 translate-y-4"
            }`}
            style={{
              ...
              pointerEvents: "auto",
              transform: `translate3d(0, ${localOnboardingActive ? "0" : "16px"}, 0)`,
            }}
            type="button"
            aria-label={
          ...
          >
            ...
            <div className="absolute bottom-full right-1/2 transform translate-x-1/2 mb-2 px-3 py-1 bg-gray-900 dark:bg-gray-700 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 ease-out whitespace-nowrap pointer-events-none scale-95 group-hover:scale-100">
              {currentStep === steps.length - 1
                ? "Finish Tour"
                : `Next (${currentStep + 2}/${steps.length})`}
            </div>
          </button>
          ...
          <div
            ref={tooltipRef}
            className={`relative z-[10000] transition-all pointer-events-auto duration-300 ease-out ${
              localOnboardingActive ? "opacity-100 scale-100" : "opacity-0 scale-95"
            }`}
            style={{
              top: 0,
              left: 0,
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden backdrop-blur-sm relative">
              ...
              <div className={isMobile ? "p-5" : "p-6"}>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    onClick={handleClose}
                    className="relative z-[10002] text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-all duration-150 ease-out p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl hover:scale-110 hover:rotate-90"
